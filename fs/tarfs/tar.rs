...
use kernel::{c_str, prelude::*, str::CString, uaccess};
...
impl TarFs {
    fn iget(sb: &SuperBlock<Self>, ino: u64) -> Result<ARef<INode<Self>>> {
        ...
        inode.init(inode::Params {
            ...
            mode: mode & 0o777,
            size: size.try_into()?,
            blocks: (size + TARFS_BSIZE - 1) / TARFS_BSIZE,
            nlink: 1,
            ...
            |data| {
                for e in DirEntry::from_bytes_to_slice(data).ok_or(EIO)? {
                    ...
                    if name_len > name.len() {
                        name.extend_with(name_len - name.len(), 0, GFP_NOFS)?;
                    }
