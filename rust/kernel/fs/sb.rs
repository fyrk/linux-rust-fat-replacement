...
impl<T: FileSystem + ?Sized> SuperBlock<T, New> {
    ...
    pub fn min_blocksize(&mut self, size: i32) -> i32 {
        ...
        // SAFETY: This a new superblock that is being initialised, so it's ok to set the block
        // size. Additionally, we've checked that the superblock is backed by a block device,
        // so it is also valid.
