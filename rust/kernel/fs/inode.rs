...
impl<T: FileSystem + ?Sized> New<T> {
    ...
    pub fn init(self, params: Params<T::INodeData>) -> Result<ARef<INode<T>>> {
        ...
        inode.i_ctime_sec = params.ctime.seconds();
        inode.i_ctime_nsec = params.ctime.nanoseconds();
        inode.i_mtime_sec = params.mtime.seconds();
        inode.i_mtime_nsec = params.mtime.nanoseconds();
        inode.i_atime_sec = params.atime.seconds();
        inode.i_atime_nsec = params.atime.nanoseconds();
